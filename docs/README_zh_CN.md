...
- **修复输入法 (IME) Bug**: 修复了在使用输入法（如日语输入）时，确认输入的 Enter 键被错误处理为换行的问题。
- **UI 改进**: 在弹出菜单中添加了切换开关，可在“黑名单模式（默认开启）”和“白名单模式（默认关闭）”之间切换。
- **国际化增强**: 改进并应用了所有 34 种支持语言的翻译。

### v1.3.2 (2026-02-12)

- **站点适配器架构**: 重写了内部引擎，完全隔离了特定于站点的逻辑。这防止了修复一个站点导致另一个站点崩溃的问题。详情请参阅 [架构说明](ARCHITECTURE_zh_CN.md)。
- **新增支持站点**: 添加了对 Gemini, Grok, DeepSeek, Perplexity, Telegram, Discord, Slack, Microsoft Teams, WeChat, z.ai 等的支持。
- **入门指南与UI**: 改进了入门指南的文本和布局。
- **国际化**: UI全面本地化。
